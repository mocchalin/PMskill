...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---------|---------|-----------|
| 文档无法读取（损坏/加密/未知编码） | 尝试用通用编码（UTF-8 / GBK）重读；docx 可尝试 unzip 修复 | 提示用户重新上传或转换为其他格式 |
| 文档结构混乱（无标题层级、全篇一段） | 按段落分组建立扁平索引；按关键词粗分章节 | 在回复中告知用户"文档结构不清晰，匹配精度可能下降" |
| 解析后内容为空 | 检查文件是否真的是需求文档（可能是图片/扫描件） | 提示用户确认文件内容或提供文本版本 |
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---------|---------|-----------|
| 单条反馈匹配到多个位置（歧义） | 按关键词密度+章节相关性排序，取 Top 1；在 CHECKPOINT 中列出歧义项请用户确认 | 若用户未指定，将该反馈标记为"多位置匹配"并跳过执行 |
| 反馈内容无法在文档中找到任何匹配 | 扩大搜索范围到全文关键词匹配；尝试语义近义词匹配 | 归入"无法匹配"列表，在 CHECKPOINT 中建议新增到"补充需求"章节 |
| 操作类型无法判断（补充/修改/删除模糊） | 根据"是否有原文"+"是否有建议修改"推断；优先假设为"修改" | 在 CHECKPOINT 中标注"类型待确认"，请用户明确 |
| 匹配位置在表格/代码块等特殊区域 | 检查表格保护规则和 Markdown 特殊元素处理表 | 若不确定如何安全标记，在 CHECKPOINT 中标注"需人工处理" |
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---------|---------|-----------|
| docx XML 编辑后文件无法打开 | 检查 XML 标签是否闭合、书签 ID 是否冲突；用 `zip -r` 重新打包时确保 `[Content_Types].xml` 完整 | 回退到编辑前备份，改用纯文本批注方式告知用户修改建议 |
| Markdown 插入 `<span>` 后渲染异常 | 检查 `<span>` 是否在代码块/行内代码/链接内部；检查 HTML 标签是否正确闭合 | 移除异常 span，改用纯文本标记（如 `[更新]` 前缀） |
| 标记颜色未生效（紫色不显示） | docx 检查 `w:color` 值是否为 `7B2D8E`；Markdown 检查 span style 语法 | 保留文本变更本身，在变更摘要中注明"颜色标记因格式限制未生效" |
| 修改位置在文档边缘（首行/末行/表格边界） | 格外小心不破坏文档边界结构；首行修改保留 frontmatter / 封面 | 若边界操作风险过高，跳过该处并在 CHECKPOINT 报告中说明 |
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---------|---------|-----------|
| 文档中找不到目录/TOC位置 | 搜索 `[TOC]` / `[[toc]]` 标记；搜索手写目录块（连续 `- [xxx](#yyy)`） | 在文档第一个标题之前插入变更摘要 |
| 书签 ID 与已有书签冲突 | 扫描文档已有书签 ID，使用全局唯一递增 ID（如从 1000 开始） | 放弃书签跳转机制，变更摘要中仅用文字描述位置 |
| 变更摘要中的超链接/锚点失效 | 检查 `w:anchor` 与 `w:name` 是否一致；Markdown 检查 `#change-N` 与 `<a id>` 是否对应 | 移除超链接，保留纯文本摘要 |
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---------|---------|-----------|
| 找不到已有修订历史表 | 按关键词搜索（修订记录/Revision History/Change Log 等）；docx 搜索表格第一行关键词 | 自动创建新表（详见下方「如果文档中没有修订历史表」章节） |
| 版本号格式无法识别或为空 | 使用默认 `V1.1`；如果历史记录全空则使用 `V1.0` | 在修订内容中注明"版本号待补充" |
| 表格追加行后格式错乱 | 复制已有行的 `trPr` / `tcPr` 结构，只替换文本 | 若表格结构异常严重，在文档末尾以段落形式追加修订说明 |
| 修订历史表列数与预期不符 | 按实际列数填充；缺失列留空 | 只填写版本号/日期/修订人/修订内容四列，其余留空 |

---

## 文档修订历史表

> 本节是第六步的详细实现参考，包含各格式的识别方法和代码示例。
...
> 本节是第四步的详细实现参考，包含各格式表格操作的代码示例和检查清单。

**核心原则：永远不破坏文档中已有的表格结构。** 更新时必须在表格内部按单元格粒度精确操作，严禁增删行列、合并/拆分单元格、改变表格边框和样式等结构性变更。
...
> 本节是第五步的详细实现参考，包含各格式的书签+超链接机制代码。

**核心规则**：变更摘要使用**黑色普通文字**，不使用紫色；每条修改记录必须带**超链接（书签跳转）**，点击即可定位到文档中对应的紫色修改内容。
